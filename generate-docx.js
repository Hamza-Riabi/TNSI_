...
function sectionHeading(text, level = 1) {
  const size = level === 1 ? 28 : 26; // 14pt ou 13pt
...
const content = [
  ...
  sectionHeading("2. SCÉNARIOS UTILISATEURS"),

  sectionHeading("Scénario 1 : Candidature en Data Analyst", 2),
  normalParagraph("Acteur : Hamza, candidat | Contexte : Renault propose un poste de Senior Data Analyst (SQL, PowerBI, datalake)"),
  normalParagraph("Flux :", true),
  bulletPoint("Hamza exécute : /pdf https://renault.jobs/offers/1234"),
  bulletPoint("Le système extrait le JD, détecte l'archétype Data Analyst, récupère le CV source, réordonne les expériences"),
  bulletPoint("Hamza valide l'aperçu texte"),
  bulletPoint("Génère /output/cv-hamza-renault-2026-05-25.pdf (1 page, ~45 KB)"),
  bulletPoint("Hamza postule directement via le portail"),
  normalParagraph("Valeur créée : CV taillé sur mesure en <2 min, mots clés du JD visibles aux ATS et recruteurs.", true),

  new Paragraph({ children: [new TextRun("")], spacing: { after: 240 } }),

  sectionHeading("Scénario 2 : Candidature en Chef de Projet IT", 2),
  normalParagraph("Acteur : Hamza, candidat | Contexte : Startup propose un poste de PM Data Engineering (Agile, Scrum, livraison, backlog)"),
  normalParagraph("Flux :", true),
  bulletPoint("Hamza exécute : /pdf https://startup.apply.com/pm-data-eng"),
  bulletPoint("Le système extrait le JD, détecte l'archétype Chef de Projet IT, met en avant 3D Smartfactory"),
  bulletPoint("Intègre user stories, backlog, sprint dans les bullets de management"),
  bulletPoint("Aperçu texte validé"),
  bulletPoint("Génère PDF avec framing orienté gestion / livraison"),
  normalParagraph("Valeur créée : Montre une capacité à livrer dans les délais, soft skills mis en avant, CV aligné management.", true),

  new Paragraph({ children: [new TextRun("")], spacing: { after: 240 } }),

  sectionHeading("Scénario 3 : Candidature en Analytics Engineer", 2),
  normalParagraph("Acteur : Hamza, candidat | Contexte : DataOps propose un poste de Data Pipeline Engineer (ETL, Kafka, PostgreSQL, Hive)"),
  normalParagraph("Flux :", true),
  bulletPoint("Hamza exécute : /pdf https://dataops.career/pipeline-engineer"),
  bulletPoint("Le système extrait le JD, détecte l'archétype Analytics Engineer"),
  bulletPoint("Met en avant le projet Pipeline end-to-end et HDFS & Hive"),
  bulletPoint("Intègre orchestration ETL, data quality, modélisation SQL"),
  bulletPoint("Génère PDF avec forte tonalité technique Data Engineering"),
  normalParagraph("Valeur créée : Profil data technique mis en lumière, keywords d'infrastructure data positionnés.", true),

  new Paragraph({ children: [new PageBreak()] }),

  // 3. ARCHITECTURE LOGICIELLE
  sectionHeading("3. ARCHITECTURE LOGICIELLE"),

  sectionHeading("3.1 Schéma d'architecture", 2),
  normalParagraph("Pipeline de transformation : URL → Extraction JD → Analyse Claude → Adaptation → HTML → PDF", true),

  new Paragraph({
    children: [new TextRun({
      text: "┌─ Claude Code ─ /pdf <url> ─┐ → WebFetch/Playwright → Claude API → CV Source → Pipeline d'Adaptation → HTML Generation → Playwright PDF → /output/cv-*.pdf",
      font: "Courier New",
      size: 20
    })],
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({
        children: [
          createTableCell("CLI Interface"),
          createTableCell("Slash command /pdf <url>, validation UI"),
          createTableCell("Claude Code Skill")
        ]
      })
    ]
  }),
  ...
  normalParagraph("Succès = ", true),
  bulletPoint("MVP fonctionnel fin juin"),
  bulletPoint("3 archétypes testés"),
...
async function generateDocument() {
  ...
  sections: [{
    properties: {
      page: {
        margin: { top: 1440, right: 1440, bottom: 1440, left: 1440 } // 1 inch = 2.54cm
      }
    },
    footers: {
      default: new Footer({
